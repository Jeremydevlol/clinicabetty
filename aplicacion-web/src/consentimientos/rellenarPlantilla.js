...
/**
 * Para PDF con cabecera ya dibujada (datos del paciente + título): elimina el bloque repetido
 * (Datos del acto, rejilla ASCII, etc.) y deja el cuerpo a partir de la primera sección típica.
 * Si no hay TECNICA/RIESGOS, devuelve el texto completo (plantillas cortas o genéricas).
 */
export function cuerpoConsentimientoParaPdf(textoRellenado) {
  const t = String(textoRellenado || "").replace(/\r\n/g, "\n").trim()
  /** Tras el bloque estándar "Datos del acto … ---". */
  const sep = t.match(/\n---\s*\n/)
  let work = sep && sep.index != null ? t.slice(sep.index + sep[0].length).trim() : t
  /** Orden: variantes con tilde, TECNICA, riesgos, otras secciones típicas. */
  const cortes = [
    /\bTÉCNICA\s*:/iu,
    /\bTECNICA\s*:/i,
    /\bPOSIBLES RIESGOS\s*:/i,
    /\bRIESGOS\s*:/i,
  ]
  for (const re of cortes) {
    const m = re.exec(work)
    if (m && m.index != null) return work.slice(m.index).trim()
  }
  return work
}
